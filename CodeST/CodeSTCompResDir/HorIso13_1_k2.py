...
#comparação entre Fibras na Horizontal com Isotrópico 0.53
#Os plots ficarão aqui, e quando quiser plotar um, deixo o outro comentado
#Fibras na Horizontal usando o Diretório

#Antes de rodar, verificar do Isotrópico K2 ST, e Horizontal, e esse códigod:
#Tamanho de malha
...
#plt.title('HorIso13_1_k2, com '+ str(nr) + ' volumes paralelos em r', fontsize=20)
...
for i in range(len(eixox)):
    Diff_1_k1_k2.append(math.fabs((D_13_1_k2_RLine2[i] - D_13_1_FH_RLine2[i]))/ D_13_1_k2_RLine2[i] * 100)

PercDiff = Diff_1_k1_k2.copy()
plt.ylabel('Valores percentuais', fontsize=25)
plt.plot(eixox, PercDiff)
#plt.title('HorIso13_1_k2, , Malha de '+ str(nr) + ' volumes', fontsize=20)
plt.xlabel('Eixo r [m]', fontsize=21)
plt.xticks(fontsize=20)
plt.legend(fontsize=20)
plt.yticks(fontsize=20)
