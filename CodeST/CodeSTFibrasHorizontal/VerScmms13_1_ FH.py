...
ColunaTumorComp = 51
D_Mms_13_1_FH_RLine = D_Mms_13_1_FH[:, ColunaTumorComp]
#SolTmms13_1_k1
...
plt.plot(eixox, D_Mms_13_1_FH_RLine, marker='o', label='Solução pelo Mms')
...
#plt.title('VerScmms13_1_FH, , Malha de '+ str(nr) + ' volumes', fontsize=20)
...
#estrutura para comparação, Tira uma linha paralela a r
#Importante atualizar o valor de ColunaTumorComp para ((nz/2) + 1)
# Usar o valor de D_Mms_13_1_FH_RLine na comparação
D_Mms_13_1_FH = D.copy()
#usando a coluna que tem o tumor. Neste código não tem tumor, mas para ficar no padrão dos que tem
# Valor de ColunaTumorComp = (nz/2) + 1
ColunaTumorComp = 51
...
A_1RLine = A_1[:, ColunaTumorComp]
Diff_SolScMms_13_1_FH = []
eixox = numpy.linspace(ra, R, nr)
for i in range(len(eixox)):
    Diff_SolScMms_13_1_FH.append(math.fabs((D_Mms_13_1_FH_RLine[i] - A_1RLine[i]))/ D_Mms_13_1_FH_RLine[i] * 100)

PercDiff = Diff_SolScMms_13_1_FH.copy()
...
plt.title('VerScmms13_1_FH, Malha de '+ str(nr) + ' x ' + str(nz) + ' volumes, Matrizes Sobrepostas')
plt.ylabel("Valores ")
plt.legend()
plt.show()
"""
