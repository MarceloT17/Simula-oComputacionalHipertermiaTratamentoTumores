...
#Olhe aqui
#Antes de rodar, verificar do Isotrópico K1 T, e Vertical, e nesse codigo:
#Tamanho de malha
...
#plt.title('VetIso13_2_k1, com '+ str(nr) + ' volumes paralelos em r', fontsize=20)
...
for i in range(len(eixox)):
    Diff_1_k1_k2.append(math.fabs((D_13_2_k1_RLine2[i] - D_13_2_FV_RLine2[i]))/ D_13_2_k1_RLine2[i] * 100)

PercDiff = Diff_1_k1_k2.copy()
plt.ylabel('Valores percentuais', fontsize=25)
plt.plot(eixox, PercDiff)
#plt.title('VetIso13_2_k1, , Malha de '+ str(nr) + ' volumes', fontsize=20)
plt.xlabel('Eixo r [m]', fontsize=21)
plt.yticks(fontsize=20)
plt.xticks(fontsize=20)
plt.show()
